...
class PoseEngine:
    ...
    def get_keypoints(self, image_path):
        ...
        if is_side:
            start_y = temp_joints["hip_left"][1]
            end_y = temp_joints["knee_left"][1]
            max_width = 0
            best_l, best_r = None, None
            for y in range(start_y, end_y, 5):
                l, r = self.get_visual_boundary(person_mask, y, center_x)
                if l is not None:
                    width = r - l
                    if width > max_width:
                        max_width = width
                        best_l, best_r = [l, y], [r, y]
            res_downscaled["hip_boundary_left"] = best_l
            res_downscaled["hip_boundary_right"] = best_r
            res_downscaled["view_type"] = "side"
        else:
            start_y_h = temp_joints["hip_left"][1]
            end_y_h = temp_joints["knee_left"][1]
            max_width_h = 0
            best_l_h, best_r_h = None, None
            for y in range(start_y_h, end_y_h, 5):
                l, r = self.get_visual_boundary(person_mask, y, center_x)
                if l is not None:
                    width = r - l
                    if width > max_width_h:
                        max_width_h = width
                        best_l_h, best_r_h = [l, y], [r, y]
            res_downscaled["hip_boundary_left"] = best_l_h
            res_downscaled["hip_boundary_right"] = best_r_h
            res_downscaled["view_type"] = "front"
